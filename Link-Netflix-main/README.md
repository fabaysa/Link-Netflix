...
## 7. Website Netflix Token Generator

Project ini kini memiliki antarmuka Website yang terintegrasi dengan Vercel & Supabase yang sama:

- **URL Web Portal**: `https://domain-anda.vercel.app/`
- **Alur Kerja**:
  1. Pengguna membuka web dan menempelkan cookies Netflix.
  2. Frontend memanggil `POST /api/generate` untuk memasukkan job ke antrian Supabase (`gemini_checker_jobs`).
  3. Worker Vercel (`api/bridge-worker.js`) mengklaim job dan mengirim cookies ke bot target melalui MTProto Userbot.
  4. Bot target membalas (link NFT Token atau peringatan Cookie Expired).
  5. Hasil disimpan ke Supabase (`result`).
  6. Frontend mem-poll status dari `GET /api/job-status?id=...` dan menampilkan:
     - **Sukses**: Tombol **Buka** dan **Salin** untuk setiap device (🖥️ PC / Laptop, 📱 HP / Mobile, 📺 TV / Smart TV), tanggal expired, dan catatan penting.
     - **Cookie Expired / Tidak Valid**: Penjelasan terstruktur mengenai kemungkinan penyebab dan saran mengambil cookie baru.

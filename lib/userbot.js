...
function isTerminalBotReply(text) {
  if (!text) return false;
  const lower = text.toLowerCase();

  // Success NFT Token link complete reply
  const hasToken = lower.includes("nftoken=") || lower.includes("token berhasil");
  const hasDeviceOrExpired = lower.includes("expired") || lower.includes("device") || lower.includes("pc");
  if (hasToken && hasDeviceOrExpired) return true;

  // Cookie expired / invalid complete reply
  const hasCookieError = lower.includes("cookie") && (lower.includes("expired") || lower.includes("tidak valid"));
  const hasSolution = lower.includes("coba lagi") || lower.includes("ambil cookie");
  if (hasCookieError && hasSolution) return true;
 ...
 * Optimized with fastPredicate for instant return on button appearance.
 */
async function waitForBotMessages(client, afterId, timeoutMs = 10000, fastPredicate = null) {
  ...
  while (Date.now() < deadline) {
    ...
    if (incoming.length) {
      // Instant return if fast predicate matches (e.g. buttons arrived)
      if (fastPredicate && incoming.some(fastPredicate)) {
        return incoming;
      }
...
export async function sendRelayText(client, payload) {
  ...
  // Step 2: Wait for bot's /start reply (fast return as soon as button is available)
  console.log(`[relay] Step 2 — Waiting for /start reply...`);
  const startReplies = await waitForBotMessages(
    client,
    startSentId,
    10000,
    m => Boolean(m.replyMarkup)
  );
  ...
  // Step 4: Wait briefly for bot to be ready for cookies
  console.log(`[relay] Step 4 — Waiting for post-button readiness...`);
  try {
    await waitForBotMessages(client, btnResult.waitAfterId, 2500);
    console.log(`[relay] Post-button prompt received`);
  } catch {
    console.log(`[relay] Proceeding directly to send cookies`);
  }
...
// ---------------------------------------------------------------------------
// Wait for target reply (final step — optimized with instant terminal return)
// ---------------------------------------------------------------------------
...
export async function waitForTargetReply(
  ...
  sentMessageId,
  {
    ...
    pollMs = Number.parseInt(process.env.RELAY_POLL_MS || "350", 10),
    settleMs = Number.parseInt(process.env.RELAY_SETTLE_MS || "600", 10)
  } = {}
) {
  const target = targetUsername();
  const deadline = Date.now() + Math.max(10_000, timeoutMs);
  const safePollMs = Math.max(250, pollMs);
  const safeSettleMs = Math.max(300, settleMs);
  ...
  while (Date.now() < deadline) {
    ...
    if (incoming.length) {
      const combinedText = incoming.map(messageText).filter(Boolean).join("\n\n");

      // Instant return if terminal response (success token or cookie error) is complete!
      if (isTerminalBotReply(combinedText)) {
        console.log(`[relay] Terminal reply detected immediately (${incoming.length} msgs)`);
        return {
          messages: incoming.map(serializeIncomingMessage),
          resultMessageId: messageId(incoming.at(-1)),
...
export async function testTargetConversation(client, timeoutMs = 30000) {
  ...
  // Test koneksi: kirim /start langsung tanpa multi-step relay flow.
  // Ini hanya untuk mengecek apakah bot target merespon.
  const startSent = await client.sendMessage(target, { message: "/start" });
  const startSentId = messageId(startSent);
  if (!startSentId) {
    return { ok: false, reason: "Gagal mengirim /start ke target bot." };
  }

  try {
    const reply = await waitForTargetReply(client, startSentId, {
      timeoutMs,
      settleMs: 1200
    });
